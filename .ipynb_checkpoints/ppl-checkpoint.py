...
class Config:
    # wandb params
    dataset_config_path: str = "random.yaml"
    project: str = "IPL"
    group: str = "random"
    name: str = "ipl"
    # model params
    actor_learning_rate: float = 0.0003
    critic_learning_rate: float = 0.00005
    hidden_dim: int = 256
    ...
    cuda: str = "0"
    save_every: int = 50
    CKPT_DIR_actor: str = "ckpts_acto"
    CKPT_DIR_critic: str = "ckpts_criti"
    w: int = 8
...
class PotentialTrainState(TrainState):
    target_params: FrozenDict
...
def update_actor(
    ...
    def actor_loss_fn(params: jax.Array) -> Tuple[jax.Array, Metrics]:
        ...
        potential_values = potential.apply_fn(potential.params, batch["states"], actions)

        loss = (beta * bc_penalty - lmbda * q_values).mean() - potential_values.mean()
...
def update_potential(
    ...
    policy_actions = actor.apply_fn(actor.target_params, batch["states"])
    data_actions = batch["actions"]

    def potential_loss_fn(potential_params: jax.Array) -> Tuple[jax.Array, jax.Array]:
        potential_policy = potential.apply_fn(potential_params, batch["states"], policy_actions)
        potential_data = potential.apply_fn(potential_params, batch["states"], data_actions)
        potential_loss = potential_policy.mean() - (W * potential_data).mean()
        return potential_loss

    (loss), grads = jax.value_and_grad(potential_loss_fn, has_aux=False)(
        potential.params
    )
    new_potential = potential.apply_gradients(grads=grads)
    new_metrics = metrics.update(
        {
            "potential_loss": loss,
        }
    )
    return key, new_potential, new_metrics
...
def update_td3(
    ...
    key, new_actor, new_critic, new_metrics = update_actor(
        key, actor, new_critic, potential, batch, actor_bc_coef, tau, normalize_q, new_metrics
    )
    
    key, new_potential, new_metrics = update_potential(
        key, new_actor, potential, batch, W, new_metrics,
    )
    
    return key, new_actor, new_critic, new_potential, new_metrics
...
def update_td3_no_targets(
    ...
    return key, actor, new_critic, potential, new_metrics
...
def main(config: Config):
    ...
    wandb.init(
        ...
        config=dict_config,
        group=config.group,
        name=f"{config.name}_{config.dataset_name}",
    )
    ...
    key, actor_key, critic_key, potential_key = jax.random.split(key, 4)
    ...
    #Using critic hyperparams
    potential_module = Potential(
        hidden_dim=config.hidden_dim,
        layernorm=config.critic_ln,
        n_hiddens=config.critic_n_hiddens,
    )
    potential = PotentialTrainState.create(
        apply_fn=potential_module.apply,
        params=potential_module.init(potential_key, init_state, init_action),
        target_params=potential_module.init(potential_key, init_state, init_action),
        tx=optax.adam(learning_rate=config.critic_learning_rate / 100), 
    )
    ...
    def td3_loop_update_step(i: int, carry: TrainState):
        ...
        key, new_actor, new_critic, new_potential, new_metrics = jax.lax.cond(
            update_carry["delayed_updates"][i], full_update, update
        )

        carry.update(key=key, actor=new_actor, critic=new_critic, potential=new_potential, metrics=new_metrics)
        return carry
    ...
    for epoch in trange(config.num_epochs, desc="IPL Epochs"):
        ...
        wandb.log(
            {"epoch": epoch, **{f"IPL/{k}": v for k, v in mean_metrics.items()}}
        )
        
        if epoch % config.save_every == 0 or epoch == config.num_epochs - 1:
            checkpoints.save_checkpoint(ckpt_dir=config.CKPT_DIR_actor+f"/ipl_{config.w}_{epoch}_{config.train_seed}_{config.dataset_name}", target=actor, step=epoch)
            checkpoints.save_checkpoint(ckpt_dir=config.CKPT_DIR_critic+f"/ipl_{config.w}_{epoch}_{config.train_seed}_{config.dataset_name}", target=critic, step=epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_config_path', type=str, default="random.yaml")
    parser.add_argument('--train_seed', type=int, default=0)
    parser.add_argument('--w', type=int, default=8)
    parser.add_argument('--cuda', type=str, default="0")
    parser.add_argument('--CKPT_DIR_actor', type=str, default="ckpts_actor")
    parser.add_argument('--CKPT_DIR_critic', type=str, default="ckpts_critic")
    ...
    dataset_config = yaml.safe_load(Path(dict_of_params['dataset_config_path']).read_text())
    dataset_config.update(dict_of_params)
    dataset_config['group'] = f"ipl_{dataset_config['dataset_name']}_w_{dataset_config['w']}"
    dataset_config['project'] = "IPL"

    config = Config(**dataset_config)
    print(config)
    
    os.environ["CUDA_VISIBLE_DEVICES"] = vars(args)["cuda"]
    main(config)
